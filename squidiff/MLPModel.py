...
class MLPModel(nn.Module):
    ...
    def __init__(self, 
        ...
        if use_encoder: 
            self.encoder = EncoderMLPModel(gene_size,hidden_sizes, num_classes)
    ...
    def forward(self, x, timesteps=None, **model_kwargs):
        ...
        if self.use_encoder: 
            if 'z_mod' in model_kwargs.keys():
                ...
                z_sem = self.encoder(model_kwargs['x_start'],label = model_kwargs['cond'])
            else:
...
class EncoderMLPModel(nn.Module):
    ...
    def __init__(self, input_size, hidden_sizes, num_classes=None, output_size=60, dropout=0.1, use_fp16=False):
        ...
        if num_classes is not None: 
            self.fc1 = nn.Linear(input_size+hidden_sizes, hidden_sizes)
        else: 
